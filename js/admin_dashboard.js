...
async function updateAdminProfile(event) {
  ...
  const adminDescription = document
...
async function followUp(index) {
  try {
    const response = await fetchData(`/customers/follow-up/${index}`, {
      method: "POST",
    });
        ...
        .map(
          (customer, index) => `
            <div style="color: ${customer.unpaidAmount > 0 ? "red" : "black"}">
                ${customer.name} (${
            customer.email
          }) - Date of Service Booked: ${customer.serviceDate} - ${
            customer.unpaidAmount > 0
              ? "Unpaid: $" + customer.unpaidAmount
              : "Paid"
          }
                ${
                  customer.unpaidAmount > 0
                    ? `<button onclick="followUp(${index})">Follow Up</button>`
                    : ""
                }
            </div>
        `
...
document
  ...
  .addEventListener("click", modifyService);
document
